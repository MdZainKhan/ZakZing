...
const ShopContextProvider = (props)=>{
    ...
    useEffect(()=>{
        fetch('https://zakzingbackend.onrender.com/allproducts')
        .then((response)=>response.json())
        .then((data)=>setAll_Product(data))

        if(localStorage.getItem('auth-token')){
            fetch('https://zakzingbackend.onrender.com/getcart',{
                method:'POST',
                headers:{
    ...
    const addToCart = (itemId)=>{
        ...
        if(localStorage.getItem('auth-token')){
            fetch('https://zakzingbackend.onrender.com/addtocart',{
                method:'POST',
                headers:{
                    Accept:'application/form-data',
                    'auth-token':`${localStorage.getItem('auth-token')}`,
    ...
    const removeFromCart = (itemId)=>{
        setCartItems((prev)=>({...prev,[itemId]:prev[itemId]-1}))
        if(localStorage.getItem('auth-token')){
            fetch('https://zakzingbackend.onrender.com/removefromcart',{
                method:'POST',
                headers:{
                    Accept:'application/form-data',
                    'auth-token':`${localStorage.getItem('auth-token')}`,
